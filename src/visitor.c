...
// ast_t* VisitorVisitVariable(visitor_t* self, astVariable_t* node, scope_t* scope, bool noerror);
// ast_t* VisitorVisitCall(visitor_t* self, astCall_t* node, scope_t* scope);
// ast_t* VisitorVisitAddress(visitor_t* self, astValue_t* node, scope_t* scope);
...
//             node->base.dtypeInfo.ptrCount != var->base.dtypeInfo.ptrCount)) {
...
//             node->base.dtypeInfo.ptrCount != node->value->dtypeInfo.ptrCount) {
...
//     // @? = ...
//     if (node->deref) {
//         printf("[Visitor]: TODO: Trying to dereference an undefined variable '%s'\n", node->name);
//         exit(1);
//     }
...
//             var->base.dtypeInfo.ptrCount != node->value->dtypeInfo.ptrCount) {
//             printf( "[Visitor]: TODO: Incompatible conversion "
//                     "assigning to '%s' from '%s'\n", 
//                     dTypeToStr(node->base.dtypeInfo.dtype),
//                     dTypeToStr(node->value->dtypeInfo.dtype));
...
//     if (scope->mom) {
//         scope->stackIndex -= dtypeInfoSize(var->base.dtypeInfo);
...
//         node->base.children->items[i] = VisitorVisitVariable(self, child, node->scope, true);
...
// ast_t* VisitorVisitAddress(visitor_t* self, astValue_t* node, scope_t* scope) {
//     node->value = VisitorVisit(self, node->value, scope);
//     node->base.dtypeInfo = node->value->dtypeInfo;
//     if (node->ref) {
//         node->base.dtypeInfo.ptrCount++;
//     } else {
//         if (node->base.dtypeInfo.ptrCount == 0) {
...
//         case AST_VARIABLE:      return VisitorVisitVariable(self, (astVariable_t*) node, scope, false);
//         case AST_CALL:          return VisitorVisitCall(self, (astCall_t*) node, scope);
//         case AST_ADDRESS:       return VisitorVisitAddress(self, (astValue_t*) node, scope);
